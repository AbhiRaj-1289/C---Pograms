// C program to find sum of only even numbers between two inputted numbers.
...
void main()
{
    ...
    {
        if(i % 2 == 0)                          // condition to  get only even numbers
        {
            sum = sum + i;                      // adding the final numbers
        }
    }

    printf("The sum of the even numbers between %d and %d including them is %d",x,y,sum);    // final output with result
}
